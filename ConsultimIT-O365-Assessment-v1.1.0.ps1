...
function Invoke-IdentityChecks {
    ...
    $guestUsers  = @()
    try {
        Write-Dbg "API: Get-MgBetaUser -All (requesting Id,DisplayName,UPN,AccountEnabled,UserType,SignInActivity,CreatedDateTime,AssignedLicenses)" -Level "API"
        $allUsers = Get-MgBetaUser -All -ConsistencyLevel eventual `
            -Property "Id,DisplayName,UserPrincipalName,AccountEnabled,UserType,SignInActivity,CreatedDateTime,AssignedLicenses" `
            -ErrorAction Stop
        ...
        Write-Dbg "Users fetched: Total=$($allUsers.Count)  Enabled/Members=$($enabledUsers.Count)  Guests=$($guestUsers.Count)  Licensed=$($licensedUsers.Count)" -Level "OK"
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
    ...
    <div class="card">
      ...
      <table class="info-table" style="width:100%">
        ...
        <tr><td>Tenant ID</td>         <td style="font-size:12px;font-family:monospace">$($Script:TenantId)</td></tr>
        <tr><td>Initial Domain</td>    <td>$($Script:Stats.InitialDomain)</td></tr>
        <tr><td>Country</td>           <td>$($Script:Stats.Country)</td></tr>
        <tr><td>Tenant Created</td>    <td>$($Script:Stats.TenantCreated)</td></tr>
        <tr><td>Verified Domains</td>  <td>$($Script:Stats.VerifiedDomains)</td></tr>
        ...
        <tr><td>Tool Version</td>      <td>ConsultimIT-O365-Assessment v1.1.0</td></tr>
      </table>
    </div>

    <div class="card">
      <div class="card-title">User & Identity Stats</div>
      <table class="info-table" style="width:100%">
        <tr><td>Total Users</td>            <td><strong>$($Script:Stats.TotalUsers)</strong></td></tr>
        <tr><td>Enabled Members</td>        <td>$($Script:Stats.EnabledUsers)</td></tr>
        <tr><td>Guest Users</td>            <td>$($Script:Stats.GuestUsers)</td></tr>
        <tr><td>Licensed Users</td>         <td>$($Script:Stats.LicensedUsers)</td></tr>
        <tr><td>Unlicensed Users</td>       <td>$($Script:Stats.UnlicensedUsers)</td></tr>
    ...
    <div class="card">
      ...
      <table class="info-table" style="width:100%">
        ...
        <tr><td>Service Principals</td>     <td>$($Script:Stats.ServicePrincipals)</td></tr>
        <tr><td>Microsoft Secure Score</td> <td>$secureScoreDisplay</td></tr>
      </table>
    </div>

    <div class="card">
      <div class="card-title">Assessment Score</div>
      <table class="info-table" style="width:100%">
        <tr><td>Tool Secure Score</td>  <td><strong>$($Scores.ToolScore)%</strong> (Grade: <strong>$($Scores.Grade)</strong>)</td></tr>
        <tr><td>Risk Level</td>         <td><strong style="color:${riskColor}">$($Scores.RiskLevel)</strong> (Score: $($Scores.RiskScore)/100)</td></tr>
        <tr><td>Total Checks</td>       <td>$($Scores.TotalChecks)</td></tr>
        <tr><td>Passed</td>             <td style="color:#27ae60;font-weight:700">$($Scores.PassCount)</td></tr>
        <tr><td>Failed</td>             <td style="color:#c0392b;font-weight:700">$($Scores.FailCount)</td></tr>
        <tr><td>Warnings</td>           <td style="color:#f39c12;font-weight:700">$($Scores.WarnCount)</td></tr>
        <tr><td>Informational</td>      <td>$($Scores.InfoCount)</td></tr>
      </table>
    </div>

  </div>

  $(if ($Script:OrgObject -and $Script:OrgObject.VerifiedDomains) {
    $domRows = ($Script:OrgObject.VerifiedDomains | Sort-Object Name | ForEach-Object {
        $flags = @()
        if ($_.IsDefault)  { $flags += "Default" }
        if ($_.IsInitial)  { $flags += "Initial" }
        if ($_.IsVerified) { $flags += "Verified" }
        "<tr><td style='padding:10px 16px;border-bottom:1px solid var(--border);'>$($_.Name)</td><td style='padding:10px 16px;border-bottom:1px solid var(--border);'>$($_.Type)</td><td style='padding:10px 16px;border-bottom:1px solid var(--border);'>$($flags -join ', ')</td></tr>"
    }) -join "`n"
    @"
  <div class="card" style="margin-top:20px;">
    <div class="card-title">Verified Domains</div>
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr>
        <th style="padding:10px 16px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.6px;color:var(--text-muted);font-weight:700;border-bottom:2px solid var(--border);">Domain</th>
        <th style="padding:10px 16px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.6px;color:var(--text-muted);font-weight:700;border-bottom:2px solid var(--border);">Type</th>
        <th style="padding:10px 16px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:.6px;color:var(--text-muted);font-weight:700;border-bottom:2px solid var(--border);">Flags</th>
      </tr></thead>
      <tbody>$domRows</tbody>
    </table>
  </div>
...
function Main {
    ...
    if ($DebugMode -and $Script:DebugLog.Count -gt 0) {
        ...
        $separator = [string]::new('=', 70)
        $lines = [System.Collections.Generic.List[string]]::new()
        $lines.Add("ConsultimIT-O365 Debug Log — Generated $((Get-Date).ToString('yyyy-MM-dd HH:mm:ss'))")
        $lines.Add($separator)
        $lines.Add("")
        foreach ($entry in $Script:DebugLog) {
            $lines.Add("[$($entry.Time)] [$($entry.Level.PadRight(5))]  $($entry.Message)")
        }
